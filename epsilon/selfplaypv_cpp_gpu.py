...
encode_action = encode_action_cpp  # use the c++ encoder

def fmt_player(p: int) -> str:
    if p == SUN:
        return "SUN"
    if p == MOON:
        return "MOON"
...
def make_cpp_eval_fn(net: LinithPVNet, device: torch.device, use_amp: bool = True):
    """
    Returns eval_fn(arg):

      - If arg is a single LinithEnv:
          -> (policy_vector, value)

      - If arg is a list/tuple of LinithEnv:
          -> list[(policy_vector, value), ...]  (for batched C++ MCTS)

    This lets pv_mcts_search_cpp do batched evals on GPU.
    """
    ...
    def eval_single(env: LinithEnv):
        ...
        with torch.no_grad():
            with autocast_ctx():
                logits, value = net(obs_t)        # logits: [1, ACTION_SIZE], value: [1,1]
            logits = logits[0]
    ...
    # ---- batched path (called from pv_evaluate_batch_cpp) ----
    def eval_batch(env_list):
        # env_list is a Python list of LinithEnv from C++
        obs_list = []
        for env in env_list:
        ...
        with torch.no_grad():
            with autocast_ctx():
                logits, values = net(obs_t)       # logits: [B,A], values: [B,1]

        logits = logits.float()
        policies = torch.softmax(logits, dim=1).cpu().numpy().astype(np.float32)
        values = values.squeeze(-1).float().cpu().numpy()  # [B]

        out = []
        for i in range(policies.shape[0]):
            out.append((policies[i], float(values[i])))
        return out

    # ---- dispatcher ----
    def eval_fn(arg):
        # scalar env from old code
        if not isinstance(arg, (list, tuple)):
            return eval_single(arg)
        # batched envs from new C++ loop
        return eval_batch(arg)
...
def generate_self_play(
    ...
    # DEBUG: announce what this worker is actually using
    print(
        f"[selfplay] device={dev}, "
        f"torch.cuda.is_available={torch.cuda.is_available()}, "
        f"torch.version.cuda={torch.version.cuda}"
    )
    if dev.type == "cuda":
        try:
            print(f"[selfplay] cuda device 0 = {torch.cuda.get_device_name(0)}")
        except Exception as e:
            print(f"[selfplay] cuda get_device_name failed: {e}")

    # Let cuDNN pick the best kernels if we’re on GPU
    if dev.type == "cuda":
    ...
    net.to(dev)
    net.eval()
    eval_fn = make_cpp_eval_fn(net, dev)  # this will now be smarter about GPU below
    ...
    for g in range(games):
        ...
        try:
            ...
            while not env.state.done:
                ...
                # Simple per-move debug line
                # print(
                #     f"[G{game_index:03} M{move_idx:04}] "
                #     f"player={fmt_player(env.state.current_player)} "
                #     f"mode={mode} "
...
def main():
    ap = argparse.ArgumentParser(description="Self-play generator for Linith PV net (C++ env).")
    ...
    ap.add_argument("--out", type=str, default="selfplay_pv_cpp_iter1.npz")
    ap.add_argument("--replay-capacity", type=int, default=0, help="Max positions to keep in replay buffer (0 = keep all)")
    args = ap.parse_args()
