...
def make_cpp_eval_fn(net: LinithPVNet, device: torch.device):
    """
    eval_fn used by pv_mcts_search_cpp.

    Supports BOTH:
      - eval_fn(env)          -> (policy_vector, value)
      - eval_fn([env, ...])   -> [(policy_vector, value), ...]

    All NN work is done on the GPU in batches.
    """
    net.to(device)
    net.eval()

    @torch.inference_mode()
    def eval_fn(env_or_envs):
        # -------------------------
        # Batched path: list/tuple
        # -------------------------
        if isinstance(env_or_envs, (list, tuple)):
            ...
            batch = torch.stack(obs_list, dim=0).to(device, non_blocking=True)
            logits, values = net(batch)  # [B, ACTION_SIZE], [B] or [B,1]

            probs = torch.softmax(logits, dim=1)
            probs_np = probs.cpu().numpy().astype(np.float32)
            values_np = values.view(-1).cpu().numpy().astype(np.float32)

            out = []
            for i in range(len(env_or_envs)):
                out.append((probs_np[i], float(values_np[i])))
            return out
        ...
        obs = env.state.to_tensor()
        obs_np = np.asarray(obs, dtype=np.float32)
        batch = torch.from_numpy(obs_np).unsqueeze(0).to(device, non_blocking=True)
                ...
                if not use_mcts:
                    ...
                    if tau != 1.0:
                        ...
                        if not np.isfinite(s) or s <= 0.0:
                            # Fallback: uniform over legal moves only
                            pi_temp[:] = 0.0
                            if legal_indices:
                                p = 1.0 / len(legal_indices)
                                for _, idx in legal_indices:
                                    pi_temp[idx] = p
                        else:
                            pi_temp /= s
                    else:
